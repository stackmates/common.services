...
module.exports = function user(options) {
  ...
  seneca.add({
    ...
    id:{required$:true,string$:true} // login token
  }, cmd_auth )
  ...
  seneca.add({
    ...
    id:{required$:true,string$:true} // user id
  }, cmd_logout )
  ...
  function cmd_auth(args,done){
    ...
    // seneca.log.debug('user id from token',args.id);
    // get decoded jsonwebtoken
    var q = {id:args.id}
    ...
    loginent.load$(q, function( err, login ) {
      ...
      if( !login ) {
        return done(null,{ok:false,id: args.id,why:'login-not-found'})
      }
  ...
  // Logout an existing user - resolve using id, idempotent
  // - token:    login token
  // Provides:
  // - success: {ok:true,user:,login:}
  function cmd_logout(args,done){
    ...
    loginent.load$(q, function( err, login ){
      ...
      if( !login ) {
        seneca.log.debug('cannot find user login',user,login)
  ...
  // WHY? When is it ever a good idea to let this info out?
